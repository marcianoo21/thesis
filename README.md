...
graph TD
    A[Użytkownik] -->|Wiadomość| B[chat_interface.py]
    B -->|User Input| C[ConversationalRAG]
    
    C -->|1. Ekstrakcja Query| D[PLLuMLLM]
    D -->|"Keyword: 'pizzeria'"| C
    
    C -->|2. Wyszukiwanie| E[Search Function]
    E -->|Encode Query| F[RoBERTa Model]
    F -->|Query Embedding| G[FAISS Index]
    
    H[(Embeddings JSONL)] -->|Load| G
    G -->|Top K Results| E
    E -->|Results List| C
    
    C -->|3. Historia + Query + Results| D
    D -->|"Prompt with Context"| I[Hugging Face API]
    I -->|PLLuM-12B| D
    D -->|Generated Response| C
    
    C -->|4. Update History| J[Conversation History]
    C -->|Response| B
    B -->|Display| A
